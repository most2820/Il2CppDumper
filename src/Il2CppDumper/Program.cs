...
namespace Il2CppDumper
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"ERROR: Failed to load config.json: {e.Message}");
            ...
            {
                if (il2cppPath == null && !Console.IsInputRedirected)
                {
        ...
        private static int ReadFatSelection(int count)
        {
            while (true)
            {
                try
        ...
        private static ulong ReadHexUInt64()
        {
            while (true)
            {
                ...
                if (ulong.TryParse(line, System.Globalization.NumberStyles.HexNumber, null, out var value))
                {
                    return value;
                }
                Console.Write("Invalid hex number, try again: ");
            }
        }
        ...
        private static bool Init(string il2cppPath, string metadataPath, out Metadata metadata, out Il2Cpp il2Cpp)
        {
            ...
            switch (il2cppMagic)
            {
                ...
                case 0xBEBAFECA:
                    ...
                    Console.WriteLine();
                    var index = ReadFatSelection(machofat.fats.Length);
            ...
            try
            {
                ...
                if (!flag)
                {
                    ...
                    var codeRegistration = ReadHexUInt64();
                    Console.Write("Input MetadataRegistration: ");
                    var metadataRegistration = ReadHexUInt64();
