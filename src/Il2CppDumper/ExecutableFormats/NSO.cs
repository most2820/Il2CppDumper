    ...
    public sealed class NSO : Il2Cpp
    {
        ...
        public NSO(Stream stream) : base(stream)
        {
            ...
            if (!IsCompressed)
            {
                ...
                var modPointer = ReadUInt32();
                // Legacy builds store an offset relative to the start of .text;
                // newer SDKs store an absolute VA and the module header may live in .ro.
                var modVa = (ulong)header.TextSegment.MemoryOffset + modPointer;
                if (!IsMod0(modVa))
                {
                    modVa = modPointer;
                    if (!IsMod0(modVa))
                    {
                        throw new InvalidDataException("ERROR: Can't locate the module header (MOD0) of this NSO.");
                    }
                }
                Position = MapVATR(modVa) + 4;
                var dynamicOffset = modVa + ReadUInt32();
                var bssStart = (uint)(modVa + ReadUInt32());
                var bssEnd = (uint)(modVa + ReadUInt32());
                ...
                var maxSize = (long)((header.DataSegment.MemoryOffset + header.DataSegment.DecompressedSize - dynamicOffset) / 16);
        ...
        private bool IsMod0(ulong va)
        {
            try
            {
                Position = MapVATR(va);
                return ReadUInt32() == 0x30444F4D; //MOD0
            }
            catch
            {
                return false;
